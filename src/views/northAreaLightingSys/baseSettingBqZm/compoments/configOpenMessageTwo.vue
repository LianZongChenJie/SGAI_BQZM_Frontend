<template>
  <div class="add-modal-wrapper">
    <a-modal
      ...
      :destroyOnClose="true"
      :maskClosable="false"
      wrapClassName="config-open-two-modal"
    >
           <!-- 顶部提示条 -->
...
function showModal(initObj) {
  console.log('type', initObj.type);
  console.log('row', initObj.row);
  type.value = initObj.type;
  Object.assign(rowObj, initObj.row)
  open.value = true;
}
...
/* ==================== config-open-two-modal 弹框样式（通过唯一类名隔离，不污染全局） ==================== */
<style lang="less">
.config-open-two-modal {
